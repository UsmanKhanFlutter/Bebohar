...
class _PostJobScreenState extends State<PostJobScreen> {
  ...
  Future<void> _post() async {
    ...
    Provider.of<Customer>(context, listen: false).postJobWithImage(_image == null ? '' : _image.path, _postData).then((value) {
      Navigator.of(context).pop();
      showDialog(
        context: context,
        builder: (ctx) => AlertDialog(
          content: Text(value ? 'Job added successfully' : 'Failed try again later'),
          actions: <Widget>[
            FlatButton(
              onPressed: () {
      ...
      child: ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          return ListTile(
            onTap: () {
              setState(() {
                _postData['category_name'] = subCategories[index].name;
                _postData['category_id'] = subCategories[index].id;
              });
            ...
            children: [
              Card(
                ...
                child: ListTile(
                                    ...
                                    ListTile(
                                      onTap: () async {
                                        if (categories[i].subCategories > 0) {
                                          Provider.of<Customer>(context, listen: false).getSubCategories(categories[i].id);
                                          await _showSubCategoryDialog(subCategories);
                                        } else {
                                          setState(() {
                                            _postData['category_id'] = categories[i].id;
                                            _postData['category_name'] = categories[i].name;
                                          });
                  ...
                  title: Text(
                    _postData['category_name'].isEmpty ? 'Select Categories' : _postData['category_name'],
                  ...
                  child: Column(
                    children: [
                      ...
                      Padding(
                        ...
                        //   onTap: () async {
                        //     Navigator.of(context).pushNamed(SetLocationScreen.routeName).then((value) {
                        //       setState(() {
                        ...
                        child: GestureDetector(
                          onTap: () {
                            DatePicker.showDateTimePicker(
                              ...
                              onConfirm: (time) {
                                final String formatted = '${DateFormat.yMd().format(time)} ${DateFormat.jm().format(time)}';
                                setState(() {
              ...
              Padding(
                ...
                child: Container(
                  height: 100,
                  width: 100,
                  child: _image == null || _image.path.isEmpty ? Center() : Image.file(File(_image.path)),
                ),
              ),
            ],
          ),
